...
FPS_video = 60
display_text = "Initialize app"
pygame.init()

os.environ['SDL_VIDEO_CENTERED'] = '1'

font = pygame.font.Font("font/zh-cn.ttf", 18)

video_load = cv2.VideoCapture("media/loading_video.mp4")
success, img = video_load.read()
video_shape = img.shape[1 :: -1]

wn = pygame.display.set_mode(video_shape, pygame.NOFRAME)
...
def splash_video():
    ...
    while success:
        ...
        if not success:
            video_load.set(cv2.CAP_PROP_POS_FRAMES, 0)
            success, img = video_load.read()
        ...
        if success:
            ...
            wn.blit(pygame.image.frombuffer(img.tobytes(), video_shape, "BGR"), (0, 0))

            text_surface = font.render(display_text, True, (255, 255, 255))
            text_rect = text_surface.get_rect()
            font_shape = font.size(display_text)
            text_rect.center = (video_shape[0] - font_shape[0] // 2 - 10, video_shape[1] - font_shape[1] // 2 - 10)
            wn.blit(text_surface, text_rect)

        pygame.display.update()


splash_app = threading.Thread(target = splash_video)
splash_app.start()

time.sleep(0.25)

try:
    display_text = "Loading module: Tkinter"
    import tkinter as tk
    from tkinter import *
except:
	display_text = "Downloading module: Tkinter"
	subprocess.check_call([sys.executable, "-m", "pip", "install", "tkinter"])
	from tkinter import *

try:
    display_text = "Loading module: math"
    import math as m
except ImportError:
    display_text = "Downloading module: math"
    subprocess.check_call([sys.executable, "-m", "pip", "install", "math"])
    import math as m
	
try:
    display_text = "Loading module: sympy"
    from sympy import *
except ImportError:
    display_text = "Downloading module: sympy"
    subprocess.check_call([sys.executable, "-m", "pip", "install", "sympy"])
    from sympy import *
	
try:
    display_text = "Loading module: fractions"
    from fractions import *
except ImportError:
    display_text = "Downloading module: fractions"
    subprocess.check_call([sys.executable, "-m", "pip", "install", "sympy"])
    from fractions import *
	
try:
    display_text = "Loading module: re"
    import re as r
except ImportError:
    display_text = "Downloading module: re"
    subprocess.check_call([sys.executable, "-m", "pip", "install", "re"])
    import re as r

try:
    display_text = "Loading module: random"
    import random
except ImportError:
    display_text = "Downloading module: random"
    subprocess.check_call([sys.executable, "-m", "pip", "install", "random"])
    import random

try:
    display_text = "Loading module: json"
    import json
except ImportError:
    display_text = "Downloading module: json"
    subprocess.check_call([sys.executable, "-m", "pip", "install", "json"])
    import json

try:
    display_text = "Loading module: string"
    import string
except ImportError:
    display_text = "Downloading module: string"
    subprocess.check_call([sys.executable, "-m", "pip", "install", "string"])
    import string

try:
    display_text = "Loading module: webbrowser"
    import webbrowser
except ImportError:
    display_text = "Downloading module: webbrowser"
    subprocess.check_call([sys.executable, "-m", "pip", "install", "webbrowser"])
    import webbrowser

try:
    display_text = "Loading module: concurrent"
    import concurrent.futures
except ImportError:
    display_text = "Downloading module: concurrent"
    subprocess.check_call([sys.executable, "-m", "pip", "install", "concurrent"])
    import concurrent.futures

try:
    display_text = "Loading module: cryptography"
    from cryptography.hazmat.primitives.asymmetric import rsa, padding
    from cryptography.hazmat.primitives import serialization, hashes
except ImportError:
    display_text = "Downloading module: cryptography"
    subprocess.check_call([sys.executable, "-m", "pip", "install", "cryptography"])
    from cryptography.hazmat.primitives.asymmetric import rsa, padding
    from cryptography.hazmat.primitives import serialization, hashes

display_text = "Initialize data"
version_of_calc = "Version 1.5.2.95"
name_of_calc = "Casio fx-880btg Emu"
url_help = "https://docs.google.com/forms/d/e/1FAIpQLSfRlg5Wo8wQpya0evQ00vT6waKZlB-eNCyYJY4q9T8ApKBemw/viewform?usp=sf_link"
DATA_FILE = "data/variable_data.json"
private_key_path = "data/private_key.pem"
public_key_path = "data/public_key.pem"
signature_path = "data/signature.sig"
key = ""
language = "English"
...
[f"Norm!RATIO-ON!{norm_txt}", ["Norm 1 : 1.23 × 10^-3",
                               "Norm 2 : 0.00123"]]]],
["Engineer Symbol (ES)", ["ES : On!RATIO-OFF!",
                    	  "ES : Off!RATIO-ON!"]]]],
...
[["Screen", ["400 × 750!RATIO-ON!",
 			 "480 × 1100!RATIO-OFF!",
			 "500 × 720!RATIO-OFF!"]],
["Fix function performance (FFP)", ["FFP : On!RATIO-OFF!",
									"FFP : Off!RATIO-ON!"]]]]
]
...
display_text = "Function initialization"
...
def change_language():
	...
	if language == "English":
		...
		Setting_select[3][0] = "Quality - Efficiency"
		Setting_select[3][1][0][0] = "Screen"
		Setting_select[3][1][1][0] = "Fix function performance (FFP)"
		...
		Setting_select[3][0] = "Chất lượng - Hiệu suất"
		Setting_select[3][1][0][0] = "Màn hình"
		Setting_select[3][1][1][0] = "Sửa lỗi hiệu suất hàm (FFP)"
		...
		Setting_select[3][0] = "质量 - 效率"
		Setting_select[3][1][0][0] = "屏幕"
		Setting_select[3][1][1][0] = "修复功能性能 (FFP)"
		...
		Setting_select[3][0] = "品質 - 効率"
		Setting_select[3][1][0][0] = "画面"
		Setting_select[3][1][1][0] = "関数のパフォーマンスを修正 (FFP)"
...
def simp(in_exp):
	...
	in_exp += sqrt(0)
	in_exp = simplify(in_exp)
	terms = in_exp.as_ordered_terms()
	...
	for i in range(0, len(terms)):
		...
		if len(numer.as_ordered_terms()) >= 3:
			return fix_result(round(N(in_exp), fix_num))
		
		denom_terms = denom.as_ordered_terms()

		if len(denom_terms) >= 3:
			return fix_result(round(N(in_exp), fix_num))
		
		if len(denom_terms) == 1:

			if "sqrt" in str(denom_terms[0]):
				numer, denom = numer * denom, denom * denom
			
			terms[i] = simplify(numer/denom)
		
		if len(denom_terms) == 2:
			
			conjugate = denom_terms[0] - denom_terms[1]
			numer, denom = simplify(numer * conjugate), simplify(denom * conjugate)
	...
	cache_terms = []
	for i in terms:
		term = i.as_ordered_terms()
		for j in term:
			cache_terms.append(j)
	
	terms = cache_terms
	in_exp = sum(terms)

	cache_exp = radsimp(together(in_exp))

	if ("/" in str(cache_exp)) and ("." in str(cache_exp)):
		return fix_result(round(N(in_exp), fix_num))
	
	cache_exp_numer, cache_exp_denom = cache_exp.as_numer_denom()
	cache_exp_numer, cache_exp_denom = cache_exp_numer.as_ordered_terms(), cache_exp_denom.as_ordered_terms()

	if len(cache_exp_denom) >= 2:
		return fix_result(round(N(in_exp), fix_num))
	
	if abs(N(cache_exp_denom[0])) >= max_const:
		return fix_result(round(N(in_exp), fix_num))

	for term in cache_exp_numer:

		term = term.as_ordered_factors()
		...
		if len(term) == 1:
			...
			if "sqrt" in str(term[0]):

				if abs(N(term[0]**2)) >= max_sqrt:
					return fix_result(round(N(in_exp), fix_num))
				
			if ("sqrt" not in str(term[0])) and (len(cache_exp_numer) != 1):

				if abs(N(term[0])) >= max_const:
					return fix_result(round(N(in_exp), fix_num))
		
		if len(term) == 2:

			if "sqrt" in str(term[1]):

				if abs(N(term[1]**2)) >= max_sqrt:
					return fix_result(round(N(in_exp), fix_num))
				
				if abs(N(term[0])) >= max_coeff:
					return fix_result(round(N(in_exp), fix_num))
	...
	if ("sqrt" in str(in_exp)) and ("pi" not in str(in_exp)):
		...
		if len(constant_terms) == 0:
			...
			return str(radsimp(together(in_exp)))
			...
			if make_frac < 100:
				...
				out = radsimp(together(out/make_frac))
				return out
...
def Sqrt(n, sq = 2):
	...
	n, sq = fix_result(eval(n)), fix_result(eval(sq))
...
def power(a, b):
	...
	a, b = fix_result(eval(a)), fix_result(eval(b))

	if b * log(a, 10) >= log(limit_high, 10):
		return None
...
def update_variable():
	...
	global ans, mode, imag, x, y, z, Catalog_select, Setting_select, language, fix_out, sci_out, fix_txt, sci_txt, norm_txt, fix_num, deci, ES_mode, key, width_screen, height_screen, FFP_mode
	...
	FFP_mode = saved_data.get("FFP_mode")
	if FFP_mode == "ON":
		FFP_solve()
...
def setting_handle(inp):
	...
	global Setting_select, address_select, mode, fix_out, sci_out, min_scroll, max_scroll, select_scroll, sci_txt, fix_txt, norm_txt, Setting_select, limit_low_result, fix_num, ES_mode, language, cache_lang, calc_mode, height_screen, width_screen, max_length_exp, FFP_mode
	...
	if "FFP : On" in inp:
		...
		for i in range(len(Setting_select[3][1][1][1])):
			Setting_select[3][1][1][1][i] = Setting_select[3][1][1][1][i].replace("RATIO-ON", "RATIO-OFF")
		
		Setting_select[3][1][1][1][0] = Setting_select[3][1][1][1][0].replace("RATIO-OFF", "RATIO-ON")
		select_scroll = address_select[-1]
	...
	if "FFP : Off" in inp:
		...
		Setting_select[3][1][1][1][1] = Setting_select[3][1][1][1][1].replace("RATIO-OFF", "RATIO-ON")
		select_scroll = address_select[-1]
			...
			if calc_mode == "Calculating":
				...
				if calc_mode == "Calculating":
					...
					if calc_mode_cache == "Normal":
						...
						out2.config(text = "= " + str(output).replace("×", ""))
						out1.config(text = scr_exp(expression))
...
display_text = "Initialize the GUI"

app = tk.Tk()
app.geometry(f"{width_screen}x{height_screen}")
app.resizable(False, False)
app.title(f"{name_of_calc} ~ {version_of_calc}")
icon_app = PhotoImage(file = "media/icon.png")
...
display_text = "Initiate behavior"
...
display_text = f"Complete in {round(time.time() - time_start_app, 3)}s"
time.sleep(0.5)
success = False
video_load.release()
pygame.quit()
